...
```{r}
na_Prevalence_of_Undernourishment_all <- sum(is.na(life.expectancy_all_years$Avg_Prevalence_of_Undernourishment))
na_CO2_all <- sum(is.na(life.expectancy_all_years$Avg_CO2))
na_Health_Expenditure_all <- sum(is.na(life.expectancy_all_years$Avg_Health_Expenditure))
na_Unemployment_all <- sum(is.na(life.expectancy_all_years$Avg_Unemployment))
na_Sanitation_all <- sum(is.na(life.expectancy_all_years$Avg_Sanitation))
na_Injuries_all <- sum(is.na(life.expectancy_all_years$Avg_Injuries))
na_Communicable_all <- sum(is.na(life.expectancy_all_years$Avg_Communicable))
na_NonCommunicable_all <- sum(is.na(life.expectancy_all_years$Avg_NonCommunicable))
...
ggplot(life.expectancy_all_years, aes(x = Year, y = Life_Expectancy, color = Region)) + 
  geom_line(size = 1) + 
  geom_point(size = 2) +
  labs(
  ...
  scale_x_continuous(breaks = seq(min(life.expectancy_all_years$Year), 
                                  max(life.expectancy_all_years$Year), by = 1))
```
